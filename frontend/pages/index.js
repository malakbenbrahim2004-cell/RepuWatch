...
export default function RepuWatch() {
  ...
  const fetchAll = useCallback(() => {
    ...
    Promise.all([
      axios.get(`${API}/stats`),
      axios.get(`${API}/articles?limit=200`),
      ...
      <div style={css.app}>
        ...
        <aside style={css.sidebar}>
          ...
          <div style={css.sidebarBottom}>
            ...
            {["H&S Holding","H&S Group","H&S Invest","Moncef Belkhayat"].map((t,i) => (
              <div key={i} style={css.targetTag}>{t}</div>
            ))}
            <div style={css.sidebarDivider} />
            <div style={css.lastUpdate}>
              <span style={{ ...css.dot, background: pulse ? PALETTE.neu : PALETTE.pos,
        ...
        <main style={css.main}>
          ...
          <header style={css.topbar}>
            <div>
              ...
              <p style={css.pageSub}>
                Veille réputationnelle automatique — {new Date().toLocaleDateString("fr-FR",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}
              </p>
            </div>
            <button className="btn" onClick={fetchAll} style={css.refreshBtn}>
              <span style={{ display:"inline-block", animation: pulse?"spin 0.8s linear":"none" }}>↻</span>
              {" "}Actualiser
            </button>
          </header>
          ...
          {page === "dashboard" && <>
            ...
            <div style={{
              ...
              <div style={css.scoreLeft}>
                ...
                <div style={{ ...css.scoreBadge, background:c1+"22", color:c1 }}>
                  {score>=70?"● Bonne réputation":score>=40?"● Réputation neutre":"● Réputation à risque"}
                </div>
                <div style={css.scoreBarWrap}>
                  <div style={{ ...css.scoreBarFill,
                    width:`${score}%`, background:`linear-gradient(90deg,${c1},${c2})` }} />
                </div>
              </div>
            ...
            <div style={css.kpiRow}>
              {[
                ...
                <div key={i} style={{ ...css.kpi, background:k.bg, borderColor:k.color+"33" }}>
            ...
            <div style={css.chartsRow}>
                ...
                {history.length > 1 ? (
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart data={history}>
                      ...
                      <XAxis dataKey="date" tick={{ fill:PALETTE.muted, fontSize:10 }} axisLine={false} tickLine={false} />
                      <YAxis domain={[0,100]} tick={{ fill:PALETTE.muted, fontSize:10 }} axisLine={false} tickLine={false} />
                      <Tooltip
                        contentStyle={{ background:"#1E293B", border:"1px solid #334155",
                          borderRadius:8, color:PALETTE.text, fontSize:12 }}
                        formatter={v => [`${v}/100`, "Score"]}
                      />
                      <Area type="monotone" dataKey="global_score" stroke={PALETTE.primary}
                        strokeWidth={2.5} fill="url(#scoreGrad)" dot={{ fill:PALETTE.primary, r:4 }} />
                    </AreaChart>
                  </ResponsiveContainer>
                ) : (
              ...
              <div style={{ ...css.card, flex:1 }}>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={pieData} cx="50%" cy="48%" outerRadius={72}
                      innerRadius={36} dataKey="value" paddingAngle={3}
                      label={({ percent }) => `${(percent*100).toFixed(0)}%`}
                      labelLine={false}>
                      {pieData.map((e,i) => <Cell key={i} fill={e.color} />)}
                    </Pie>
                    <Legend iconType="circle" iconSize={8}
                      formatter={v => <span style={{ color:PALETTE.muted, fontSize:12 }}>{v}</span>} />
              ...
              <div style={{ ...css.card, flex:1 }}>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={topicData} layout="vertical" margin={{ left:8 }}>
                    ...
                    <YAxis type="category" dataKey="topic" tick={{ fill:PALETTE.muted, fontSize:11 }}
          ...
          {page === "articles" && (
            <div style={css.card}>
              <div style={{ ...css.cardHead, flexWrap:"wrap", gap:12, marginBottom:20 }}>
              ...
              {filtered.slice(0,100).map((art,i) => (
                <div key={i} className="article-row" style={{
                  ...
                  <div style={{ flex:4, minWidth:0 }}>
                    <a
                      ...
                      title={art.title}
                    >
                      {art.title?.length > 80 ? art.title.slice(0,80)+"…" : art.title}
                    ...
                    <div style={css.articleMeta}>
                      ...
                      {art.collected_at && (
                        <span style={css.metaDate}>
                          {new Date(art.collected_at).toLocaleDateString("fr-FR")}
                        </span>
                      )}
                      {art.is_alerted && (
                  ...
                  <div style={{ flex:1.2 }}>
                    <a href={`https://${art.domain}`} target="_blank" rel="noreferrer"
              ...
              {filtered.length > 100 && (
                <div style={css.more}>
                  + {filtered.length - 100} articles supplémentaires
                </div>
              )}
            </div>
          )}
          ...
          {page === "sources" && (
            <div style={css.card}>
              <div style={css.cardHead}>
                <span style={css.cardTitle}>{sources.length} sources détectées automatiquement</span>
                <div style={{ display:"flex", gap:16, fontSize:12, color:PALETTE.muted }}>
                  <span><span style={{ color:PALETTE.pos }}>●</span> Source connue</span>
                  <span><span style={{ color:PALETTE.neg }}>●</span> Nouvelle source</span>
                </div>
              </div>
              ...
              {sources.map((src,i) => (
                <div key={i} className="article-row" style={{
                  ...
                  <div style={{ flex:2.5, display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <a href={`https://${src.domain}`} target="_blank" rel="noreferrer"
                      style={{ color:PALETTE.text, fontWeight:500, fontSize:13,
                        textDecoration:"none" }}>
                      {src.domain}
                      <span style={css.externalIcon}>↗</span>
                    </a>
                  ...
                  <div style={{ flex:1, textAlign:"center" }}>
                    {src.is_new
                      ...
                      : <span style={{ color:PALETTE.pos, fontSize:12 }}>✓ Connue</span>}
                  </div>
          ...
          {page === "alertes" && (
            <div>
              ...
              <div style={{
                ...css.card,
                background: alertedArticles.length > 0 ? "#EF444411" : "#10B98111",
                borderColor: alertedArticles.length > 0 ? "#EF444444" : "#10B98144",
                ...
                <div>
                  <div style={{ fontSize:20, fontWeight:700,
                    color: alertedArticles.length > 0 ? PALETTE.neg : PALETTE.pos }}>
                    {alertedArticles.length > 0
              ...
              {alertedArticles.length > 0 && (
                <div style={{ ...css.card, marginBottom:24 }}>
                  <div style={css.cardHead}>
                    <span style={css.cardTitle}>Articles ayant déclenché une alerte</span>
                  </div>
                  {alertedArticles.map((art,i) => (
                    <div key={i} style={{
                      display:"flex", alignItems:"center", gap:16,
                      padding:"14px 16px", borderRadius:8, marginBottom:8,
                      background:"#EF444411", borderLeft:`3px solid ${PALETTE.neg}`,
                    }}>
                      <div style={{ flex:1 }}>
                        <a href={art.url} target="_blank" rel="noreferrer"
                          style={{ color:PALETTE.neg, fontWeight:500, fontSize:13,
                      ...
                      <span style={{ ...css.scoreTag,
                        ...
                        border:`1px solid ${PALETTE.neg}44`, fontSize:14,
                        padding:"6px 14px" }}>
                        {art.score}/100
                      </span>
                    </div>
              ...
              <div style={css.card}>
                <div style={css.cardHead}>
                  <span style={css.cardTitle}>Configuration du système d'alertes</span>
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                  {[
                    { label:"Seuil d'alerte",       value:"Score ≤ 40/100",          icon:"⚡" },
                    { label:"Canal de notification", value:"Email SMTP (Gmail)",      icon:"📧" },
                    { label:"Fréquence pipeline",    value:"Toutes les heures",       icon:"⏱" },
                    { label:"Alertes envoyées",      value:`${stats?.total_alertes} au total`, icon:"📊" },
                  ].map((c,i) => (
                    <div key={i} style={{ background:"#1E293B", borderRadius:10,
                      padding:"16px 20px", border:`1px solid ${PALETTE.border}` }}>
                      <div style={{ fontSize:22, marginBottom:8 }}>{c.icon}</div>
                      <div style={{ fontSize:12, color:PALETTE.muted, marginBottom:4 }}>{c.label}</div>
                      <div style={{ fontSize:15, fontWeight:600, color:PALETTE.text }}>{c.value}</div>
                    </div>
                  ))}
                </div>
              </div>
...
function sentStyle(s) {
  return {
    positif: { background:"#10B98122", color:"#10B981", border:"1px solid #10B98144" },
    neutre:  { background:"#F59E0B22", color:"#F59E0B", border:"1px solid #F59E0B44" },
    negatif: { background:"#EF444422", color:"#EF4444", border:"1px solid #EF444444" },
  }[s] || {};
}
...
const css = {
  app:          { display:"flex", minHeight:"100vh", background:"#0F172A", color:"#F1F5F9" },
  splash:       { display:"flex", alignItems:"center", justifyContent:"center",
                  height:"100vh", background:"#0F172A" },
  splashInner:  { textAlign:"center" },
  splashLogo:   { fontFamily:"'Syne',sans-serif", fontSize:42, fontWeight:800,
                  color:"#6366F1", letterSpacing:-1 },
  splashSub:    { color:"#94A3B8", fontSize:14, marginTop:8, marginBottom:24 },
  splashBar:    { width:200, height:3, background:"#1E293B", borderRadius:99, margin:"0 auto" },
  splashFill:   { width:"70%", height:"100%", background:"linear-gradient(90deg,#6366F1,#8B5CF6)",
                  borderRadius:99, animation:"shimmer 1.5s infinite",
                  backgroundSize:"200% 100%" },
  sidebar:      { width:240, background:"#0B1120", display:"flex", flexDirection:"column",
                  padding:"0", position:"sticky", top:0, height:"100vh",
                  borderRight:"1px solid #1E293B", flexShrink:0 },
  sidebarTop:   { padding:"28px 20px", flex:1 },
  sidebarBottom:{ padding:"20px", borderTop:"1px solid #1E293B" },
  logo:         { display:"flex", alignItems:"center", gap:12, marginBottom:32 },
  logoIcon:     { width:40, height:40, borderRadius:10, background:"linear-gradient(135deg,#6366F1,#8B5CF6)",
                  display:"flex", alignItems:"center", justifyContent:"center",
                  fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:13, color:"white",
                  letterSpacing:0.5 },
  logoText:     { fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:16, color:"#F1F5F9" },
  logoSub:      { fontSize:11, color:"#475569", marginTop:1 },
  sidebarDivider:{ height:1, background:"#1E293B", margin:"16px 0" },
  navLabel:     { fontSize:10, color:"#475569", textTransform:"uppercase",
                  letterSpacing:1.5, marginBottom:10, fontWeight:600 },
  navItem:      { display:"flex", alignItems:"center", gap:12, padding:"10px 14px",
                  borderRadius:8, color:"#64748B", fontSize:14, marginBottom:4,
                  fontFamily:"'DM Sans',sans-serif" },
  navItemActive:{ background:"#6366F122", color:"#818CF8",
                  borderLeft:"2px solid #6366F1", paddingLeft:12 },
  navIcon:      { fontSize:16, width:20, textAlign:"center" },
  navBadge:     { marginLeft:"auto", background:"#EF4444", color:"white",
                  borderRadius:99, padding:"1px 7px", fontSize:10, fontWeight:700 },
  targetTag:    { fontSize:11, color:"#6366F1", background:"#6366F111",
                  border:"1px solid #6366F133", padding:"4px 10px", borderRadius:6,
                  marginBottom:6, display:"inline-block" },
  lastUpdate:   { display:"flex", alignItems:"center", gap:8,
                  fontSize:11, color:"#475569" },
  dot:          { width:6, height:6, borderRadius:"50%", display:"inline-block", flexShrink:0 },
  main:         { flex:1, padding:"32px 36px", overflowY:"auto", maxWidth:"calc(100vw - 240px)" },
  ...
  refreshBtn:   { background:"#1E293B", color:"#94A3B8", padding:"9px 18px",
                  borderRadius:8, fontSize:13, fontWeight:500,
                  border:"1px solid #334155", cursor:"pointer" },
  scoreHero:    { border:"1px solid", borderRadius:16, padding:"28px 32px",
                  display:"flex", justifyContent:"space-between",
                  alignItems:"center", marginBottom:20 },
  scoreLeft:    { flex:1 },
  scoreLabel:   { fontSize:12, color:"#94A3B8", textTransform:"uppercase",
                  letterSpacing:1, marginBottom:10 },
  scoreNumber:  { fontFamily:"'Syne',sans-serif", fontSize:72, fontWeight:800,
                  lineHeight:1, letterSpacing:-3 },
  scoreTotal:   { fontSize:24, color:"#64748B" },
  scoreBadge:   { display:"inline-block", padding:"4px 12px", borderRadius:99,
                  fontSize:12, fontWeight:600, marginTop:10, marginBottom:16 },
  scoreBarWrap: { width:220, height:4, background:"#1E293B", borderRadius:99 },
  scoreBarFill: { height:"100%", borderRadius:99, transition:"width 1s ease" },
  scoreStats:   { display:"flex", gap:32, marginLeft:32 },
  scoreStat:    { textAlign:"center" },
  scoreStatVal: { fontFamily:"'Syne',sans-serif", fontSize:32, fontWeight:700 },
  ...
  kpi:          { borderRadius:12, padding:"20px 16px", textAlign:"center",
                  border:"1px solid" },
  kpiIcon:      { fontSize:20, marginBottom:6 },
  kpiVal:       { fontFamily:"'Syne',sans-serif", fontSize:32, fontWeight:700 },
  kpiLab:       { fontSize:11, color:"#64748B", marginTop:4 },
  chartsRow:    { display:"flex", gap:16, marginBottom:20 },
  card:         { background:"#1E293B", border:"1px solid #334155", borderRadius:14,
                  padding:"22px 24px", marginBottom:20 },
  cardHead:     { display:"flex", justifyContent:"space-between",
                  alignItems:"center", marginBottom:16 },
  cardTitle:    { fontFamily:"'Syne',sans-serif", fontSize:15, fontWeight:600, color:"#F1F5F9" },
  cardBadge:    { fontSize:11, background:"#6366F122", color:"#818CF8",
                  border:"1px solid #6366F133", padding:"3px 10px", borderRadius:99 },
  noData:       { textAlign:"center", color:"#475569", padding:"48px 0", fontSize:13 },
  ...
  sourceChip:   { display:"flex", alignItems:"center", gap:7,
                  ...
                  padding:"6px 12px", borderRadius:8, fontSize:12, cursor:"default" },
  ...
  articleRow:   { display:"flex", alignItems:"center", padding:"12px 16px",
                  borderRadius:8, gap:12, marginBottom:2 },
  articleTitle: { fontSize:13, color:"#CBD5E1", fontWeight:500,
                  display:"block", lineHeight:1.45, marginBottom:5 },
  externalIcon: { fontSize:10, color:"#6366F1", marginLeft:5, opacity:0.7 },
  ...
  alertedBadge: { fontSize:10, background:"#EF444422", color:"#EF4444",
                  border:"1px solid #EF444444", borderRadius:99, padding:"1px 7px" },
  domainLink:   { fontSize:12, color:"#6366F1", textDecoration:"none",
                  fontWeight:500 },
  topicTag:     { fontSize:11, background:"#6366F111", color:"#818CF8",
                  border:"1px solid #6366F133", padding:"3px 9px", borderRadius:6 },
  sentTag:      { fontSize:11, padding:"3px 10px", borderRadius:99, fontWeight:600 },
  scoreTag:     { fontSize:13, fontWeight:700, padding:"4px 12px", borderRadius:99 },
  searchWrap:   { position:"relative", display:"flex", alignItems:"center" },
  searchIcon:   { position:"absolute", left:12, color:"#475569", fontSize:18 },
  searchInput:  { background:"#0F172A", border:"1px solid #334155", color:"#F1F5F9",
                  padding:"9px 14px 9px 36px", borderRadius:8, fontSize:13,
                  outline:"none", width:280 },
  filterRow:    { display:"flex", gap:6, flexWrap:"wrap" },
  filterBtn:    { background:"#1E293B", color:"#64748B", padding:"8px 14px",
                  borderRadius:8, fontSize:12, fontWeight:500,
                  border:"1px solid #334155" },
  filterActive: { background:"#6366F1", color:"white", border:"1px solid #6366F1" },
  more:         { textAlign:"center", color:"#475569", padding:"16px", fontSize:13 },
};